...
void turningopperation(){
  ...
  case 0:
    ...
    if (count_distance >= 0.15){ // når bilen har kjørt en hvis lengde bytter den modus.
      turning = 1;
      reset = 1;
      previusmilllis_restart = millis();
    }
...
case 1:
  ...
  if (count_right >= 1200){ // når bilen har snudd 180 grader byttes modus, når farten er 100 snus den på 1200
    turning = 0;
    reset = 1;
    previusmilllis_restart = millis();
    if (mode == 0){
...
void wrongway(){ // unødvendig funksjon, men lagret slik for å kunne legge inn flere funksjoner senere
    turningopperation();
}
...
void readSensorvalue(){
  ...
  for (int i = 0; i < 5; i = i + 1) {
    if (lineSensorValues[i] >= min_value){ // må ha litt høy min_value pga svarte prikker på gulv, hvis disse ikka hadde hvert der så kunne min_value hver lavere og bilen ville kjørt smudere.
      on_line = 0; // modus linjeføger
    }
    }
    if (lineSensorValues[2] >= 850 and lineSensorValues[4] >= 850){  // modus snu 90 grader
...
void turningcrossroad(){ // funksjon når man skal svinge 90 grader til høyre
  
  resetdistance();  // henter inn resetdistance funksjonen
  while(reset == 0 and encoder.getCountsLeft() <= 400){ // når reset har gjort jobben *****
    motors.setSpeeds(speed*(0.5), speed*(-0.5)); // får bilen til å snu
  }

 if (encoder.getCountsLeft() >= 400 and reset == 0){ // når bilen har snudd 90 grader byttes modus, når farten er 400 snus den på 500
    reset = 1;
    previusmilllis_restart = millis();
...
void steeringleft(){  // sving litt til venstre
  for (int i = 0; i <= 100; i ++){ // tvinger bilen til å svinge til venstre
    motors.setSpeeds(speed*0.5, speed*1);
  }
  }
void steeringright(){  // sving litt til høyre
  for (int i = 0; i <= 100; i ++){
...
// hovedmodus:
//
void whatToDoMain(){  // hva skal skje i hovedmodus
  readSensorvalue(); // lese sensorverdier
  switch (on_line){ // switch funksjon, byttes med hensyn på sensorverdier

  case 0: // modus for å kjøre etter linje
    if (line_value_change == 1){ // for å starte wrong way funksjonen i riktig modus.
      line_value_change = 0;
    }
    first = 0;
  ...
  case 1:
      if (line_value_change == 0){ // første runde går den inn i funkjsonen
  ...
  case 2: 
  if (first == 0){ // går inni funksjonen første runde
        reset = 1;
        previusmilllis_restart = millis();
        first = 1;
      }
...
void main_mode(){ // funksjon som bestemmer modus
  switch (mode)
  {
